    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
        <form method="GET" action="{{ route('quotes.index') }}"
            ...
            @if(!empty($filters['search']) || !empty($filters['status']))
                <a href="{{ route('quotes.index') }}" class="btn-ghost btn-sm self-center">
                    Limpar
                </a>
            @endif
        </form>

        <a href="{{ route('quotes.create') }}" class="btn-primary whitespace-nowrap self-start sm:self-auto">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-12 text-slate-500">
                            ...
                            <a href="{{ route('quotes.create') }}" class="text-octa-400 hover:text-octa-300 ml-1">Criar o primeiro?</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
