<x-layouts.app title="Editar Contrato" header="Contratos / Editar">
    ...
    <div id="edit-contract-card" class="max-w-2xl">
        <div class="card">
            <h2 class="text-base font-semibold text-slate-200 mb-6">Editar Contrato</h2>

            <form id="form-edit-contract" method="POST" action="{{ route('contracts.update', $contract) }}" enctype="multipart/form-data">
                @csrf @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select id="select-contract-client" name="client_id"
                    ...
                    <div>
                        ...
                        <input type="number" id="input-contract-value" name="value"
                               value="{{ old('value', $contract->value) }}"
                               class="input @error('value') input-error @enderror"
                               step="0.01" min="0" required/>
                        @error('value') <p id="error-value" class="form-error">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="status" class="label">Status <span class="text-red-500">*</span></label>
                        <select id="select-contract-status" name="status"
                        ...
                        @error('status') <p id="error-status" class="form-error">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="start_date" class="label">Início <span class="text-red-500">*</span></label>
                        <input type="date" id="input-contract-start-date" name="start_date"
                               value="{{ old('start_date', $contract->start_date->toDateString()) }}"
                               class="input @error('start_date') input-error @enderror" required/>
                        @error('start_date') <p id="error-start-date" class="form-error">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="end_date" class="label">Término</label>
                        <input type="date" id="input-contract-end-date" name="end_date"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="textarea-contract-notes" name="notes" rows="3"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        @if($contract->file_path)
                            <div id="contract-file-current" class="flex items-center gap-3 mb-2 p-3 rounded-lg bg-bg-input border border-bg-border text-sm">
                                ...
                                <a id="link-download-contract" href="{{ route('contracts.download', $contract->id) }}" class="text-octa-400 hover:text-octa-300 font-medium">Baixar</a>
                            </div>
                            <p class="text-xs text-slate-500 mb-2">Selecione um novo arquivo para substituir o atual:</p>
                        @endif
                        <input type="file" id="input-contract-file" name="file" accept=".pdf,.doc,.docx"
                        ...
                        @error('file') <p id="error-file" class="form-error">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="flex items-center gap-3 mt-6 pt-4 border-t border-bg-border">
                    <button id="btn-save-contract" type="submit" class="btn-primary">Salvar Alterações</button>
                    <a id="btn-cancel-contract" href="{{ route('contracts.show', $contract) }}" class="btn-ghost">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
