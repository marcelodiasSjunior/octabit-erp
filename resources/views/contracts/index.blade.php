    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
        <form method="GET" action="{{ route('contracts.index') }}"
            ...
            @if(!empty($filters['search']) || !empty($filters['status']))
                <a href="{{ route('contracts.index') }}" class="btn-ghost btn-sm self-center">Limpar</a>
            @endif
        </form>

        <a href="{{ route('contracts.create') }}" class="btn-primary whitespace-nowrap self-start sm:self-auto">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('contracts.show', $contract) }}" class="btn-ghost btn-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0zM2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                <a href="{{ route('contracts.edit', $contract) }}" class="btn-ghost btn-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form method="POST" action="{{ route('contracts.destroy', $contract) }}"
                                      x-data @submit.prevent="if(confirm('Remover este contrato?')) $el.submit()">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center py-12 text-slate-500">
                            Nenhum contrato encontrado. <a href="{{ route('contracts.create') }}" class="text-octa-400 hover:underline">Crie o primeiro.</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
